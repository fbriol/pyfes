...
"""Comprehensive tests for the pyfes.core constituent functions."""
...
class TestConstituents:
    """Test suite for constituent parsing and querying functions."""
    ...
    def test_constituents_wave_properties_via_table(self) -> None:
        ...
        wt = core.darwin.WaveTable(['M2', 'K1', 'S2'])
    ...
    def test_constituents_wave_table_properties(self) -> None:
        """Test wave properties from WaveTable."""
        wt = core.darwin.WaveTable(['M2', 'S2', 'K1'])

        for wave in wt.waves():
            name = wave.name
            assert isinstance(name, str)
            assert name in ['M2', 'S2', 'K1']
            assert wave.frequency() > 0
            assert wave.type is not None
